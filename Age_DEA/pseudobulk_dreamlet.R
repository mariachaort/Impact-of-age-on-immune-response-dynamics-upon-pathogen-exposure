...
option_list = list(
  ...
  make_option(c("--condition"), action="store", default="CA", type="character",
  ...
  make_option(c("--phenotypes"), action="store", default="Projects/scRNAseq/mchacon/1M-scBloodNL/scripts/analysis/pseudobulk_dreamlet.phenotypes.tab", type='character',
              help="Gender/age"),
  make_option(c("--covs"), action="store", default='Projects/scRNAseq/mchacon/1M-scBloodNL/scripts/analysis/pseudobulk_dreamlet.covariates_nCells.tab',  type='character',
...
in.fn <- paste0(path_cluster, "/", in.dir, "/", chem_version , "/", cond, "/", ctype, "_", cl, "_", cond, "_", t, "_so.rds")
...
read_data <- function(in_fn = in.fn, permutation_idx = opt$permutation_idx){
  ...
  if(ncol(sce)>0){
   if(!is.null(permutation_idx)){
        ...
        donor_i.bc_real <- cell_metadata[cell_metadata$donorID==donor_i,]$barcode
        donor_i.bc_perm <- cell_metadata[cell_metadata$permutedID==donor_i,]$barcode
        ...
        cell_metadata_real_perm.fn <- paste0(out.dir, cond, "_", t ,'_cell_metadata_real_perm.rds')
        ...
        cell_metadata_perm.fn <- paste0(out.dir, cond, "_", t ,'_cell_metadata_perm.rds')
...
define_form <- function(gt, df, vp){
  ...
  cols_vars <- c(Gender.hex, Age.hex, nCells.hex, random.hex, Residuals.hex)
  names(cols_vars) <- c('Gender', 'age', 'nCells', random_var_dea, 'Residuals')
...
extract_plots <- function(i, dea_res, contrast_var, vp_res, cols, phe_dir){
  ...
  plotVolcano.fn <- paste0(phe_dir, cond, "_", t, '_plotVolcano.png')
  ...
  if(length(degs)>0){
    ...
    plotGeneHeatmap.fn <- paste0(phe_dir, cond, "_", t, '_plotGeneHeatmap.png')
    ...
    plotPercentBars.fn <- paste0(phe_dir, cond, "_", t, '_plotPercentBars.png')
    ...
    plotVarPart.fn <- paste0(phe_dir, cond, "_", t, '_plotVarPart.png')
...
dreamlet.func <- function(ge_dge, covariates, contrast_list, gene_test = c('VP','DEA'), vp_reduced = opt$vp_reduced, out_dir = out.dir){
  ...
  plotVoom.fn <- paste0(out_dir, cond, "_", t, '_plotVoom.png')
  ...
  out_fn <- paste0(out_dir, cond, "_", t, '_dea_vp_topTable.rds')
...
cnames <- c('date_lane', 'celltype', 'assignment', 'date', 'Gender', 'age')
...
system.time(pb <- aggregateToPseudoBulk(sce,
                                        ...
                                        verbose = FALSE))
pb_raw <- pb
pb_fn <- paste0(out.dir, cond, "_", t, '_pb.Rds')
saveRDS(pb, pb_fn)
